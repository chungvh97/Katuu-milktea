...
const ProductCard: React.FC<{ product: Product; onSelect: () => void; isSelected: boolean }> = ({ product, onSelect, isSelected }) => {
  ...
  return (
    <button
      onClick={onSelect}
      className={`relative w-full text-left rounded-xl overflow-hidden shadow-md transition-all duration-300 hover:shadow-xl hover:-translate-y-1 border-4 ${isSelected ? 'border-amber-500 scale-105 shadow-xl' : 'border-transparent dark:border-stone-700/50 dark:bg-stone-800'}`}
    >
      {isSelected && (
      ...
      <div className="relative w-full h-48 bg-stone-100 dark:bg-stone-700">
        {/* Skeleton Loader */}
        <div
          className={`w-full h-full bg-stone-200 dark:bg-stone-600 animate-pulse transition-opacity duration-500 ${imageLoaded ? 'opacity-0' : 'opacity-100'}`}
        />
        {/* Image */}
        <img
          src={product.image}
      ...
      <div className="p-4 bg-white dark:bg-stone-800">
        <h3 className="font-bold text-lg text-stone-700 dark:text-stone-100">{product.name}</h3>
        <p className="text-amber-600 dark:text-amber-500 font-semibold mt-1">{formatVND(product.price)}</p>
      </div>
    </button>
  );
};
...
const ProductCardSkeleton: React.FC = () => {
  return (
    <div className="w-full rounded-xl overflow-hidden border-4 border-transparent bg-white dark:bg-stone-800 shadow-md">
      <div className="w-full h-48 bg-stone-200 dark:bg-stone-700 animate-pulse" />
      <div className="p-4">
        <div className="h-5 bg-stone-200 dark:bg-stone-700 rounded w-3/4 mb-3 animate-pulse"></div>
...
const CategoryButtonSkeleton: React.FC = () => (
  <div className="h-[36px] w-24 rounded-full bg-stone-200 dark:bg-stone-700 animate-pulse"></div>
);
...
const ProductSelection: React.FC<ProductSelectionProps> = ({ products, onProductSelect, selectedProductId, selectedCategory, onCategoryChange, isLoading }) => {
  ...
  return (
    <section>
      <h2 className="text-2xl font-bold mb-6 text-stone-700 dark:text-stone-100">1. Chọn Thức Uống</h2>

      <div className="mb-8">
        <h3 className="text-xl font-semibold text-stone-600 dark:text-stone-300 mb-4">Chọn Danh Mục</h3>
        {isLoading ? (
