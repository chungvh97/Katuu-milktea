      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <button
              onClick={onBack}
              className="flex items-center text-stone-600 hover:text-amber-600 transition-colors"
            >
          ...
          <div className="flex items-center gap-3">
            <label className="text-stone-700 font-medium">Lọc theo ngày:</label>
            <select
              value={selectedDate}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg p-6">
            ...
            {stats.topProducts.length === 0 ? (
              <p className="text-stone-500 text-center py-8">Chưa có dữ liệu</p>
            ) : (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-stone-500">{product.count} món đã bán</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-green-600">{formatVND(product.revenue)}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {stats.staffRevenue.sort((a, b) => b.revenue - a.revenue).map((staff) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <p className="text-lg font-bold text-blue-600">{formatVND(staff.revenue)}</p>
                    </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mt-6">
          <h2 className="text-xl font-bold text-stone-800 mb-4 flex items-center gap-2">
            <ClockIcon className="w-6 h-6 text-stone-600" />
            Chi Tiết Đơn Hàng
          </h2>
          {(selectedDate === 'all' ? mergedOrders : (ordersByDate[selectedDate] || [])).length === 0 ? (
            <p className="text-stone-500 text-center py-8">Chưa có đơn hàng nào</p>
          ) : (
                ...
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      <span className="font-mono text-sm text-stone-500">#{order.id.split('-')[1].substring(0, 8)}</span>
                      <span className="text-xs text-stone-400">{formatDate(order.mergedAt)}</span>
                    </div>
                  ...
                  <div className="text-right">
                    <p className="text-xl font-bold text-green-600">{formatVND(order.totalPrice)}</p>
                  </div>
                </div>
              ))}
            </div>
