...
import { XIcon, HistoryIcon, EditIcon, TrashIcon, WarningIcon, ArrowLeftIcon, PackageIcon, DollarSignIcon, TrophyIcon, SearchIcon } from '../assets/icons';
...
const StatCard: React.FC<{ icon: React.ReactNode; title: string; value: React.ReactNode; }> = ({ icon, title, value }) => (
  <div className="bg-white p-6 rounded-xl shadow-sm border border-stone-200 flex flex-col items-start space-y-3 h-full">
    ...
    <div className="text-lg font-bold text-stone-800 pl-1">{value}</div>
  </div>
);
...
const OrderHistory: React.FC<OrderHistoryProps> = ({
  ...
  return (
    <main className="container mx-auto px-4 py-8 max-w-6xl animate-fade-in-up">
      <div className="mb-8">
        <button onClick={onBack} className="flex items-center text-sm font-semibold text-stone-600 hover:text-amber-600 transition-colors mb-4">
          <ArrowLeftIcon className="w-4 h-4 mr-2" />
          Quay Lại Đặt Hàng
        </button>
        <h1 className="text-4xl font-bold text-stone-800">Lịch Sử Đơn Hàng</h1>
      </div>
        ...
        <div className="p-6 border-b border-stone-200">
          ...
          <div className="mt-6 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <p className="text-sm font-semibold text-stone-600 mr-1 shrink-0">Danh Mục:</p>
              <button
            ...
            <div className="relative w-full md:max-w-xs">
              ...
              <input
                ...
                className="w-full pl-11 pr-10 py-2.5 border border-stone-300 rounded-lg focus:ring-1 focus:ring-amber-500 focus:border-amber-500 transition-all duration-300 ease-in-out bg-stone-100 hover:border-amber-300 placeholder-stone-400 focus:bg-white focus:scale-[1.01] focus:shadow-md"
              />
              {searchTerm && (
                <button
            ...
            {filteredHistory.map((order, index) => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <p className="font-bold text-stone-800">Đơn hàng của {order.customerName}</p>
                    <div className="flex items-center mt-1">
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    {isAuthenticated && user && (
                      <button
                        ...
                        className="p-1.5 rounded-full text-stone-500 hover:bg-blue-100 hover:text-blue-600 transition-all duration-200 disabled:opacity-50 disabled:pointer-events-none transform hover:scale-110 active:scale-95"
                    ...
                    <button
                      ...
                      disabled={deletingOrderId !== null || showConfirmDialog !== null}
                      className="p-1.5 rounded-full text-stone-500 hover:bg-amber-100 hover:text-amber-600 transition-all duration-200 disabled:opacity-50 disabled:pointer-events-none transform hover:scale-110 hover:rotate-6 active:scale-95"
                    ...
                    <button
                      ...
                      disabled={deletingOrderId !== null || showConfirmDialog !== null}
                      className="p-1.5 rounded-full text-stone-500 hover:bg-red-100 hover:text-red-600 transition-all duration-200 disabled:opacity-50 disabled:pointer-events-none transform hover:scale-110 hover:-rotate-6 active:scale-95"
                ...
                {order.product && (
                  <div className="border-t border-stone-100 pt-3 mt-3">
                    <p className="font-semibold text-stone-700">{order.product.name}</p>
      ...
      {showConfirmDialog && (
        ...
        >
          <div 
            className={`bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm text-center ${isDialogExiting ? 'animate-dialog-exit' : 'animate-dialog-enter'}`}
            onClick={(e) => e.stopPropagation()}
          >
             <WarningIcon className="w-16 h-16 text-red-500 mx-auto mb-4" />
             <h3 className="text-lg font-bold text-stone-800">Xóa Đơn Hàng?</h3>
             ...
             <div className="grid grid-cols-2 gap-3">
                <button
                    onClick={() => handleCloseConfirmDialog(false)}
                    className="px-4 py-2.5 bg-stone-100 text-stone-700 font-semibold rounded-lg hover:bg-stone-200 transition-colors"
                >
                    Hủy
                </button>
                <button
